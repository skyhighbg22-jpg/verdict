...
import { CheckCircle2, Archive, ExternalLink, Share2, Loader2, Download, FileText, Award, AlertTriangle, Play } from 'lucide-react';
...
const CloseOutView = () => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      // Try Airtable sync (deferred)
      const airtableResult = await bytezClient.syncToAirtable({
        summaryReport,
        finalCalibration,
        userPrompt,
    ...
    <div className="p-8 space-y-8">
      <div className="flex items-center justify-between border-b border-gray-100 pb-4">
        <div className="flex items-center gap-3 text-green-600">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm">Finalizing results and archiving anchor versions.</p>
          </div>
        </div>
        <button
      ...
      {isProcessing ? (
        <div className="flex flex-col items-center justify-center py-12 space-y-4">
          ...
          <p className="text-gray-600 font-medium">Generating final report...</p>
        </div>
      ) : (
        <>
          {/* Success Banner */}
          <div className={clsx(
            ...
            isPipelineSuccess ? "bg-green-50 border-green-100" : "bg-yellow-50 border-yellow-100"
          )}>
            <div className={clsx(
              "mx-auto w-20 h-20 rounded-full flex items-center justify-center shadow-sm border",
              isPipelineSuccess ? "bg-white text-green-600 border-green-100" : "bg-white text-yellow-600 border-yellow-100"
            )}>
              {isPipelineSuccess ? (
            ...
            <div className="flex flex-wrap justify-center gap-4">
              ...
              <button 
                ...
                className="flex items-center gap-2 px-6 py-2 bg-white text-green-600 border border-green-200 rounded-lg font-bold hover:bg-green-50 transition-colors"
              >
                <Archive size={18} />
                Archive Version
              </button>
            ...
            {exportStatus && (
              <div className={clsx(
                ...
                {exportStatus === 'success' 
                  ? 'Results exported successfully! (Airtable sync deferred to tomorrow)'
            ...
            <div className="bg-white border border-gray-100 rounded-xl p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="p-4 bg-purple-50 rounded-xl text-center">
                  <p className="text-sm text-purple-600 mb-1">Initial Score</p>
                  <p className="text-3xl font-black text-purple-700">
                    {(finalCalibration.initialScore * 100).toFixed(1)}%
                  </p>
                </div>
                <div className="p-4 bg-gray-50 rounded-xl text-center">
                  <p className="text-sm text-gray-600 mb-1">Total Penalty</p>
                ...
                <div className={clsx(
                  ...
                  finalCalibration.finalScore >= 0.7 ? "bg-green-50" : "bg-orange-50"
                )}>
                  <p className={clsx(
                    "text-sm mb-1",
                    finalCalibration.finalScore >= 0.7 ? "text-green-600" : "text-orange-600"
                  )}>
                    Final Score
                  </p>
                  <p className={clsx(
                    "text-3xl font-black",
                    finalCalibration.finalScore >= 0.7 ? "text-green-700" : "text-orange-700"
                  )}>
              ...
              {finalCalibration.breakdown && (
                <div className="mt-4 pt-4 border-t border-gray-100">
                  <p className="text-xs font-bold text-gray-500 uppercase mb-2">Penalty Breakdown</p>
                  <div className="flex gap-4">
                    <div className="flex-1">
                      <div className="flex justify-between text-sm mb-1">
                        <span className="text-gray-600">Miscalibration</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="p-6 bg-white border border-gray-100 rounded-xl shadow-sm">
              <h4 className="font-bold text-gray-800 mb-4 flex items-center justify-between">
                Integrations Status
                <span className="text-[10px] text-gray-400 uppercase">Tomorrow's Tasks</span>
              </h4>
            ...
            <div className="p-6 bg-white border border-gray-100 rounded-xl shadow-sm">
              ...
              {summaryReport ? (
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-gray-600">Skeptic Issues</span>
                    <span className="font-semibold">
                  ...
                  {summaryReport.recommendations && (
                    <div className="mt-4 pt-4 border-t border-gray-100">
                      <p className="text-xs font-bold text-gray-500 uppercase mb-2">Recommendations</p>
                  ...
                  <button className="w-full mt-4 flex items-center justify-center gap-2 px-4 py-2 bg-gray-100 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-200 transition-colors">
                    <Download size={16} />
                    Download Report
                  </button>
                </div>
                ...
                <div className="h-24 flex items-center justify-center border-2 border-dashed border-gray-100 rounded-lg">
                  <p className="text-xs text-gray-400 italic">Report generation pending...</p>
                </div>
              )}
            </div>
          </div>
          ...
          {userPrompt && (
            <div className="p-4 bg-gray-50 rounded-xl border border-gray-200">
              <p className="text-xs font-bold text-gray-500 uppercase mb-2">Original Goal</p>
              <p className="text-sm text-gray-700 italic">"{userPrompt}"</p>
            </div>
          )}
        </>
      )}
...
const IntegrationItem = ({ name, status }) => (
  <div className="flex items-center justify-between">
    ...
    <span className="text-[10px] px-2 py-0.5 bg-gray-100 text-gray-400 rounded-full font-bold uppercase">
      {status}
    </span>
  </div>
);
